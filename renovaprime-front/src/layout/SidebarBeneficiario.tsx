...
export function SidebarBeneficiario({ isOpen, onClose, collapsed, onToggleCollapse }: SidebarBeneficiarioProps) {
  ...
  useEffect(() => {
    const loadUserBeneficiaryType = async () => {
      try {
        const beneficiaries = await appointmentService.getMyBeneficiaries();
        const selfBeneficiary = beneficiaries.find((beneficiary) => beneficiary.user_id === user?.id);
    ...
    loadUserBeneficiaryType();
  }, [user?.id]);

  const navigationItems = useMemo(() => {
    if (!isDependentUser) return navigation;
    return navigation.filter(
      (item) =>
        item.href !== '/beneficiario/dependentes' &&
        item.href !== '/beneficiario/assinatura'
    );
  }, [isDependentUser]);
