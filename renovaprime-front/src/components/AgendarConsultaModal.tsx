...
export function AgendarConsultaModal({ isOpen, onClose, onSuccess }: AgendarConsultaModalProps) {
  ...
  const [selfBeneficiaryId, setSelfBeneficiaryId] = useState<number | null>(null);
  const [isDependentUser, setIsDependentUser] = useState(false);
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      const selfBeneficiary = beneficiariesData.find((beneficiary) => beneficiary.user_id === user?.id) || null;
      setSelfBeneficiaryId(selfBeneficiary?.id || null);
      setIsDependentUser(selfBeneficiary?.type === 'DEPENDENTE');
      
      if (selfBeneficiary) {
        setSelectedBeneficiary(selfBeneficiary.id);
      } else if (beneficiariesData.length === 1) {
        // fallback para cenários sem vínculo direto de user_id
  ...
  const handleAgendarConsulta = async () => {
    ...
    try {
      const date = `${selectedMonth.year}-${String(selectedMonth.month).padStart(2, '0')}-${String(selectedDay).padStart(2, '0')}`;
      const beneficiaryId = isDependentUser
        ? selfBeneficiaryId
        : (selectedBeneficiary || selfBeneficiaryId || undefined);
