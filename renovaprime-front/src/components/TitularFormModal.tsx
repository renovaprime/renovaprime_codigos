...
interface TitularFormModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const INITIAL_FORM: BeneficiaryFormData = {
  type: 'TITULAR',
  name: '',
  cpf: '',
  birth_date: '',
...
export function TitularFormModal({ isOpen, onClose, beneficiary, isEditing = false }: TitularFormModalProps) {
  ...
  const handleCEPChange = async (value: string) => {
    let cep = value.replace(/\D/g, '').substring(0, 8);
    if (cep.length > 5) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Erro ao salvar titular');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="relative bg-card rounded-2xl shadow-elevated w-full max-w-3xl max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in duration-200">
        <div className="sticky top-0 bg-card border-b border-border p-6 flex items-center justify-between">
          <h2 className="text-2xl font-semibold text-foreground">
            {isEditing ? 'Editar Titular' : 'Novo Titular'}
          </h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Input
                ...
                placeholder={isEditing ? 'Deixe em branco para manter a atual' : 'Minimo 6 caracteres'}
                required={!isEditing}
              />
              {isEditing && (
                <p className="text-xs text-muted-foreground mt-1">Deixe em branco para nao alterar a senha</p>
              )}
            </div>
            <div>
              <label className="block text-sm font-medium text-foreground mb-2">
          ...
          {/* Servico */}
          <div>
            <label className="block text-sm font-medium text-foreground mb-2">Tipo de Servico *</label>
            <select
            ...
            >
              ...
              <option value="PREMIUM">Premium (Clinico + Especialistas + Psicologia + Nutricao)</option>
