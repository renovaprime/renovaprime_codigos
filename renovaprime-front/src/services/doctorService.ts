...
class DoctorService {
  ...
  async syncDoctorWithMemed(id: number): Promise<{ synced: boolean; memed_external_id: string; message: string }> {
    const response = await apiClient.post<ApiResponse<{ synced: boolean; memed_external_id: string; message: string }>>(`/admin/doctors/${id}/sync-memed`);
    return response.data;
  }
}
