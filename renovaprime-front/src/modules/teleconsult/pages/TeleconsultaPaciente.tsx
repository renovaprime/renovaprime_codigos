...
import { ConsentModal } from '../../prontuario/components/ConsentModal';
import { medicalRecordApiService } from '../../prontuario/services/medicalRecordService';
import { appointmentService } from '../../../services/appointmentService';

const log = (...args: unknown[]) => console.log('[Teleconsult:beneficiário]', ...args);
...
export function TeleconsultaPaciente() {
  ...
  useEffect(() => {
    ...
    const checkConsent = async () => {
      try {
        // Get beneficiary ID from appointment
        const beneficiaries = await appointmentService.getMyBeneficiaries();
        if (beneficiaries.length > 0) {
          setBeneficiaryId(beneficiaries[0].id);
        }

        const consent = await medicalRecordApiService.getConsent(parseInt(appointmentId));
        ...
        // If consent check fails, proceed without blocking
        setConsentChecked(true);
      }
    };
    checkConsent();
  ...
  useEffect(() => {
    if (!appointmentId || !consentChecked) return;

    const checkAvailability = async () => {
      try {
        const result = await teleconsultService.checkAvailability(parseInt(appointmentId));
        if (result.available) {
          log('availability OK — rendering TeleconsultaRoom', { appointmentId });
        }
        ...
        log('checkAvailability error', { appointmentId, message });
  ...
  }, [appointmentId, isAvailable, consentChecked]);
  ...
  // Waiting for consent check
  if (!consentChecked) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-gray-900 text-white">
        <Loader2 className="w-12 h-12 animate-spin mb-4" />
        <p>Preparando teleconsulta...</p>
      </div>
    );
  }
