...
import type { Specialty, DoctorFormData, Profession, RegistryType, UserStatus, DocumentType, Gender } from '../types/api';
...
export function CadastroMedico() {
  ...
  const validate = (): boolean => {
    ...
    if (!formData.profession ){
      camposObrigatorios.push('Profissão');
    }
    if (!formData.registry_number ){
      camposObrigatorios.push('Número do registro');
    }
    if (!formData.registry_uf ){
      camposObrigatorios.push('UF do registro');
    }
    if (!formData.cpf ){
    ...
    if (camposObrigatorios.length > 0) {
      setError(`Preencha os campos obrigatórios: ${camposObrigatorios.join(', ')}`);
      return false;
    }
  ...
  const handleSubmit = async (configureSchedule = false) => {
    ...
    try {
      if (isEditing && id) {
        ...
        const dataToUpdate = { ...formData, cpf: formData.cpf?.replace(/\D/g, '') || '', phone: formData.phone?.replace(/\D/g, '') || '' };
        if (!dataToUpdate.password || dataToUpdate.password.trim() === '') {
          delete dataToUpdate.password;
        }
        await doctorService.updateDoctor(parseInt(id), dataToUpdate);
          ...
          <>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <Button variant="secondary" onClick={() => navigate('/profissionais')} data-cy="doctor-form-cancel">
                  Cancelar
                </Button>
                <Button onClick={() => handleSubmit(false)} disabled={isLoading} data-cy="doctor-form-submit">
                  {isLoading && <Loader2 className="w-4 h-4 animate-spin" />}
                  {isEditing ? 'Atualizar' : 'Salvar'}
                </Button>
                {!isEditing && (
                  <Button onClick={() => handleSubmit(true)} disabled={isLoading} data-cy="doctor-form-submit-schedule">
                    {isLoading && <Loader2 className="w-4 h-4 animate-spin" />}
            ...
            <Card>
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      data-cy="doctor-form-registry-uf"
                      className="w-full px-3 py-2 border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"
                    >
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      placeholder="RQE"
                      data-cy="doctor-form-rqe"
                    />
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-foreground mb-1">CPF <span className="text-red-500">*</span></label>
                    <Input
                      value={formData.cpf || ''}
                      onChange={(e) => setFormData(prev => ({ ...prev, cpf: e.target.value.replace(/\D/g, '').slice(0, 11) }))}
                      placeholder="00000000000"
                      maxLength={11}
                      data-cy="doctor-form-cpf"
                      required={true}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      data-cy="doctor-form-birth-date"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setFormData(prev => ({ ...prev, gender: (e.target.value || undefined) as Gender | undefined }))}
                      data-cy="doctor-form-gender"
                      className="w-full px-3 py-2 border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"
                    >
                      <option value="">Selecione</option>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-2 max-h-48 overflow-y-auto border border-border rounded-lg p-3" data-cy="doctor-form-specialties">
                    {specialties.map(specialty => (
                      <label key={specialty.id} className="flex items-center gap-2 cursor-pointer hover:bg-muted/30 p-2 rounded">
                        <input
                          ...
                          data-cy={`specialty-checkbox-${specialty.id}`}
                        />
                ...
                <div className="flex items-center justify-between p-4 bg-muted/30 rounded-lg">
                  ...
                  <Switch
                    ...
                    data-cy="doctor-form-approved-switch"
                  />
                </div>
              </div>
            </Card>
...
function DocumentUpload({ label, documentType, upload, onFileSelect, onRemove }: DocumentUploadProps) {
  return (
    <div data-cy={`document-upload-${documentType}`}>
      ...
      {!upload.url ? (
        <div className="relative">
          <input
            ...
            data-cy={`file-input-${documentType}`}
          />
          <label
            htmlFor={`upload-${documentType}`}
            data-cy={`upload-label-${documentType}`}
        ...
        <div className="flex items-center gap-4 p-4 border border-border rounded-lg" data-cy={`file-uploaded-${documentType}`}>
          {upload.preview ? (
            <img src={upload.preview} alt="Preview" className="w-16 h-16 object-cover rounded" data-cy="file-preview" />
          ) : (
