...
import { UserCheck, Plus, Search, Filter, Edit2, Trash2, RefreshCw, CheckCircle, AlertCircle } from 'lucide-react';
...
export function MedicosAtivos() {
  ...
  const [syncingDoctorId, setSyncingDoctorId] = useState<number | null>(null);
  ...
  const handleSyncMemed = async (doctor: Doctor) => {
    try {
      setSyncingDoctorId(doctor.id);
      setSyncMessage(null);

      const result = await doctorService.syncDoctorWithMemed(doctor.id);

      // Atualizar lista local com o novo memed_external_id
      setDoctors(prev => prev.map(d =>
        d.id === doctor.id ? { ...d, memed_external_id: result.memed_external_id } : d
      ));
      ...
      setSyncMessage({
        type: 'error',
        text: err instanceof Error ? err.message : 'Erro ao sincronizar com MEMED'
      });
      setTimeout(() => setSyncMessage(null), 5000);
    } finally {
      setSyncingDoctorId(null);
    }
  };
      ...
      <div className="space-y-6">
        <div className="relative overflow-hidden rounded-3xl border border-border/50 bg-gradient-to-br from-primary/10 via-card to-secondary/10 p-6 md:p-8">
          ...
          <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-3xl font-display font-bold text-primary md:text-4xl">Profissionais</h1>
              <p className="mt-2 text-sm text-muted-foreground md:text-base">Visualize e gerencie profissionais ativos na plataforma.</p>
            </div>
            <Button onClick={() => navigate('/profissionais/cadastro')} data-cy="new-doctor-button" className="self-start sm:self-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-3 p-2">
            <div className="relative flex-1">
              ...
              <Input
                ...
                className="pl-10"
                value={searchTerm}
                data-cy="doctor-search-input"
                onChange={(e) => setSearchTerm(e.target.value)}
              />
            </div>
            <Button variant="secondary" data-cy="doctor-filters-button">
                    ...
                    >
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            onClick={() => handleSyncMemed(doctor)}
                            disabled={syncingDoctorId === doctor.id}
                            data-cy="sync-memed-button"
                            className={`p-2 rounded-lg transition-colors ${
                              syncingDoctorId === doctor.id
                                ? 'text-muted-foreground cursor-not-allowed'
                                : doctor.memed_external_id
                                  ? 'text-muted-foreground hover:text-green-600 hover:bg-green-50'
                                  : 'text-orange-500 hover:text-orange-600 hover:bg-orange-50'
                            }`}
                            title={doctor.memed_external_id ? 'Re-sincronizar com MEMED' : 'Sincronizar com MEMED'}
                          >
                            <RefreshCw className={`w-4 h-4 ${syncingDoctorId === doctor.id ? 'animate-spin' : ''}`} />
                          </button>
                          <button
                            onClick={() => navigate(`/profissionais/editar/${doctor.id}`)}
                            data-cy="edit-doctor-button"
