...
class AvailabilityService {
  ...
  async getAvailableMonthDays(specialtyId, year, month) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const weekday = this.getWeekdayFromDate(year, month, day);
  ...
  async getAvailableSlotsForDay(specialtyId, date) {
    ...
    const weekday = this.getWeekdayFromDate(y, m, d);
    const slots = [];
  ...
  async generateDoctorSlots(doctorId, weekday, date) {
    ...
    let nowTime = null;
    if (isToday) {
      nowTime = new Intl.DateTimeFormat('sv-SE', {
   ...
   * Independe do TZ do servidor.
   */
  getTodayString() {
    const now = new Date();
    const brDate = new Intl.DateTimeFormat('sv-SE', { timeZone: 'America/Sao_Paulo' }).format(now);
    return brDate; // formato YYYY-MM-DD
  }

  /**
   * Calcula o dia da semana (0=Dom, 6=Sab) a partir de ano, mês e dia,
   * sem depender de Date timezone.
   */
  getWeekdayFromDate(year, month, day) {
    // Usar UTC explicitamente para evitar offset de timezone
