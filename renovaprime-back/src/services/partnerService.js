const { Partner, PartnerBranch, Reseller, PlanCommission } = require('../models');
...
class PartnerService {
  ...
  async listCommissions(partnerId) {
    const partner = await Partner.findByPk(partnerId);
    if (!partner) {
      throw new Error('Partner not found');
    }

    const commissions = await PlanCommission.findAll({
      where: { partner_id: partnerId },
      order: [['plan_type', 'ASC']]
    });

    const PLAN_TYPES = ['CLINICO', 'PREMIUM', 'FAMILIAR'];
    const mapped = PLAN_TYPES.map((pt) => {
      const existing = commissions.find((c) => c.plan_type === pt);
  ...
  async upsertCommissions(partnerId, items) {
    const partner = await Partner.findByPk(partnerId);
    if (!partner) {
      throw new Error('Partner not found');
    }
    ...
    for (const item of items) {
      const [commission] = await PlanCommission.findOrCreate({
        where: { partner_id: partnerId, plan_type: item.plan_type },
        defaults: {
          partner_id: partnerId,
          plan_type: item.plan_type,
          partner_pct: item.partner_pct,
          branch_pct: item.branch_pct,
          reseller_pct: item.reseller_pct,
          active: true
        }
      });

      if (commission.partner_pct !== item.partner_pct ||
          commission.branch_pct !== item.branch_pct ||
          commission.reseller_pct !== item.reseller_pct) {
