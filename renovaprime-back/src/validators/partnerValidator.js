...
const commissionItemSchema = z.object({
  plan_type: z.enum(['CLINICO', 'PREMIUM', 'FAMILIAR']),
  partner_pct: z.number().min(0).max(100),
  branch_pct: z.number().min(0).max(100),
  reseller_pct: z.number().min(0).max(100)
}).refine(
  (d) => d.partner_pct + d.branch_pct + d.reseller_pct <= 100,
  { message: 'A soma dos percentuais não pode ultrapassar 100%' }
);

const upsertCommissionsSchema = z.object({
  commissions: z.array(commissionItemSchema).min(1).max(3)
});
