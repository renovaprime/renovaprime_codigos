...
const createDoctorSchema = z.object({
  ...
  // Dados pessoais (para integração Memed)
  cpf: z.string().length(11, 'CPF must be exactly 11 digits').regex(/^\d+$/, 'CPF must contain only digits').optional(),
  birth_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Birth date must be YYYY-MM-DD').optional(),
