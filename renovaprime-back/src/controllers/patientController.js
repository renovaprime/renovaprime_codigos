...
class PatientController {
  ...
  async createDependent(req, res, next) {
    try {
      const dependent = await patientService.createDependent(req.user.id, req.body);
      return res.status(201).json(successResponse(dependent));
    } catch (error) {
      ...
      if (
        error.message === 'Dependent limit reached' ||
  ...
  async manageDependentAccess(req, res, next) {
    try {
      const result = await patientService.manageDependentAccess(
      ...
      if (
        ...
        error.message === 'Titular not found'
      ) {
        return res.status(404).json(errorResponse(error.message, 'NOT_FOUND'));
      }
  ...
  async getSubscription(req, res, next) {
    try {
      const subscription = await patientService.getMySubscription(req.user.id);
      if (!subscription) {
        return res.status(404).json(errorResponse('Subscription not found', 'NOT_FOUND'));
      }
