...
class PartnerController {
  ...
  async listCommissions(req, res, next) {
    try {
      ...
      const commissions = await partnerService.listCommissions(partnerId);
  ...
  async upsertCommissions(req, res, next) {
    try {
      const partnerId = parseInt(req.params.partnerId);
      const commissions = await partnerService.upsertCommissions(partnerId, req.body.commissions);
      return res.json(successResponse(commissions));
    } catch (error) {
      if (error.message === 'Partner not found') {
        return res.status(404).json(errorResponse(error.message, 'NOT_FOUND'));
